...
load_data = function(normalise = TRUE, remove_doublets = FALSE, remove_stripped = FALSE, load_corrected = FALSE){
  ...
  counts = readRDS(paste0(main, "03_quality_control/raw_counts.rds"))
  genes = read.table(paste0(main, "02_calledCells/genes.tsv"), stringsAsFactors = F)
    
if(file.exists(paste0(main, 'meta.tab'))){
    meta = read.csv2(paste0(main, 'meta.tab'), sep='\t')
}else if(file.exists(paste0(main, '05_doublet_detection/meta.tab'))){
        meta = read.csv2(paste0(main, '05_doublet_detection/meta.tab'), sep='\t')
}else{
         meta = read.table(paste0(main, "/saved/meta_qc.tab"), header = TRUE, sep = "\t", stringsAsFactors = FALSE, comment.char = "$")
}
  ...
  if(normalise){
    sfs = read.table(paste0(main, "saved/sizefactors.tab"), stringsAsFactors = F)[,1]
    sizeFactors(sce) = sfs
  ...
  if(load_corrected){
    corrected = readRDS(paste0(main, "corrected_pcas.rds"))
...
getHVGs = function(sce, min.mean = 1e-3, gene_df = genes){
  ...
  #exclude sex genes
    ...
    ychr = read.table("/rds/project/rds-SDzz0CATGms/users/mlnt2/PhD_MT16_WT_control/ygenes.tab", stringsAsFactors = FALSE)[,1]
...
mapWrap = function(atlas_sce, atlas_meta, map_sce, map_meta, k = 10, return.list = FALSE){
  ...
  big_sce = scater::logNormCounts(cbind(atlas_sce, map_sce))

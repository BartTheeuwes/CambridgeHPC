...
# Correction per stage skipped because this info is present in the pool, which also corrects for differences between embryo pools of the same stage
da_results <- testNhoods(Milo_sce, design = ~ sample + sample:pool + tdTom_corr, design.df = embryo_design, reduced.dim="pca")
                             
if(length(args$stage)>1){
    da_results <- testNhoods(Milo_sce, design = ~ sample:stage + tdTom_corr, design.df = embryo_design, reduced.dim="pca")
} else {
    da_results <- testNhoods(Milo_sce, design = ~ sample + tdTom_corr, design.df = embryo_design, reduced.dim="pca")
}

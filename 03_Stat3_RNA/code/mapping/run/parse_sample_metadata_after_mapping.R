...
colnames(mapping_mnn.dt) = c("cell","celltype.mapped_mnn","celltype.score_mnn","stage.mapped_mnn", "cellstage.score_mnn", "closest.cell_mnn")

to.save <- sample_metadata %>% 
  merge(mapping_mnn.dt,by="cell",all.x=TRUE)

# ############################
# ## Correct for tdTom expr ##
# ############################

# sce = readRDS(args$sce)

# # detect cells expression tdTom
# tdtom_corr = data.table(cell = colnames(sce), tdTom_corr = ifelse(as.vector(counts(sce['tomato-td']))>0, TRUE, FALSE))

# # Add to metadata
# to.save = to.save[,idx:=.I] %>%

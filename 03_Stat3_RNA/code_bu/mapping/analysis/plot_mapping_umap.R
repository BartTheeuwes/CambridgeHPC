...
stopifnot("closest.cell_mnn"%in%colnames(sample_metadata))
...
to.plot <- umap.dt %>% copy %>%
  .[,index:=match(cell, sample_metadata[,closest.cell_mnn] )] %>% 
  .[,mapped:=as.factor(!is.na(index))] %>% 
  .[,mapped:=plyr::mapvalues(mapped, from = c("FALSE","TRUE"), to = c("scRNA-seq atlas","Chimaera"))] %>%
  setorder(mapped) 
...
for (i in args$samples) {
  ...
  to.plot <- umap.dt %>% copy %>%
    .[,index:=match(cell, sample_metadata[sample==i,closest.cell_mnn] )] %>% 
    .[,mapped:=as.factor(!is.na(index))] %>% 

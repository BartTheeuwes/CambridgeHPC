...
if(file.exists(paste0(main, 'meta.tab'))){
    meta = read.csv2(paste0(main, 'meta.tab'), sep='\t')
        ...
        meta = read.csv2(paste0(main, '05_doublet_detection/meta.tab'), sep='\t')
         ...
         meta = read.table(paste0(main, "03_quality_control/meta.tab"), header = TRUE, sep = "\t", stringsAsFactors = FALSE, comment.char = "$")
}
  ...
  if(normalise){
    sfs = read.table(paste0(main, "04_normalisation/sizefactors.tab"), stringsAsFactors = F)[,1]
    sizeFactors(sce) = sfs

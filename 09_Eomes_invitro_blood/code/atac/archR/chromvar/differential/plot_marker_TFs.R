...
########################################
## Load pseudobulk chromVAR estimates ##
########################################
...
##############################################
## Load RNA vs chromVAR correlation results ##
##############################################

cor_rna_vs_chromvar.dt <- fread(paste0(io$basedir,"/results/rna_atac/rna_vs_chromvar/pseudobulk/per_gene/cor_rna_vs_chromvar_correlated_peaks_pseudobulk.txt.gz")) %>%
  .[,cor_sign:=as.factor(c("-","+")[(r>0)+1])] %>%
  .[sig==T]

##############################
## Heatmap of (celltype,TF) ##
##############################

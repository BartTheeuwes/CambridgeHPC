...
for (i in args$samples) {
  ...
  # Load cell metadata
  barcode.loc <- sprintf("%s/%s/outs/raw_feature_bc_matrix/barcodes.tsv.gz",args$inputdir,i)
  ...
  # Load matrix  
  count_mtx[[i]] <- Read10X(sprintf("%s/%s/outs/raw_feature_bc_matrix",args$inputdir,i))[["Gene Expression"]]

...
print(rownames(ArchRProject)[!rownames(ArchRProject)%in%sample_metadata$cell] %>% strsplit("#") %>% map_chr(1) %>% table)
print(sprintf("Number of cells that are derived from the ArchR project but are not present in the RNA pipeline: %s",sum(!rownames(ArchRProject)%in%sample_metadata$cell)))
print(rownames(ArchRProject)[!rownames(ArchRProject)%in%sample_metadata$cell] %>% strsplit("#") %>% map_chr(1) %>% table)

# nrow(sample_metadata[sample=="rv_eo_deg_day3_5_control"])
# nrow(sample_metadata[pass_rnaQC==T & sample=="rv_eo_deg_day3_5_control"])
tmp <- sample_metadata[sample=="rv_eo_deg_day3_5_control" & !cell%in%rownames(ArchRProject)]
tmp = sample_metadata[pass_rnaQC==T & !cell%in%rownames(ArchRProject)]

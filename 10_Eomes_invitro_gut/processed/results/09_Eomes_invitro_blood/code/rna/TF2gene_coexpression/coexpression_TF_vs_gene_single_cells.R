...
logcounts(rna_tfs.sce) <- as.matrix(logcounts(rna_tfs.sce))
logcounts(rna_genes.sce) <- as.matrix(logcounts(rna_genes.sce))
...
saveRDS(tf2gene_cor.mtx, file.path(args$outdir,"correlation_matrix_tf2gene_single_cells.rds"))

# tf2tf
tf2tf_cor.mtx <- cor(t(logcounts(rna_tfs.sce)),t(logcounts(rna_tfs.sce))) %>% round(2)
saveRDS(tf2tf_cor.mtx, file.path(args$outdir,"correlation_matrix_tf2tf_single_cells.rds"))

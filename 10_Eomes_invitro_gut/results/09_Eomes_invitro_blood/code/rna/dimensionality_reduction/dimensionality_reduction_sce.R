...
# args$features <- 2500
# args$npcs <- 50
# args$colour_by <- c("celltype","nFeature_RNA","day","genotype")
# args$vars_to_regress <- NULL # c("nFeature_RNA","mitochondrial_percent_RNA")
# args$batch_variable <- "None" # "sample"
# args$n_neighbors <- 25
# args$min_dist <- 0.5
...
if (args$stages[1]=="all") {
  args$stages <- opts$days
} else {
  stopifnot(args$stages%in%opts$days)
}
...
sample_metadata <- fread(args$metadata) %>%
  .[pass_rnaQC==TRUE & doublet_call==FALSE & day%in%args$stages & sample%in%args$samples & celltype%in%args$celltypes]
...
if (args$batch_variable=="None") {
  ...
  if (length(unique(sample_metadata[[args$batch_variable]]))==1) {
    stop(sprintf("There is a single level for %s, no batch correction can be applied",args$batch_variable))
  }
}
...
if (length(args$batch_variable)>0) {
  ...
  timepoints <- sample_metadata$day
  timepoint_order <- opts$days[opts$days%in%timepoints]
...
for (i in args$colour_by) {
  ...
  if (grepl("day",i)) {
    p <- p + scale_fill_manual(values=opts$days.colors) +

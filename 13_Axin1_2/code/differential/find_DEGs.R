...
.edgeR <- function(sce) {
  ...
  #if(length(intersect(unique(meta[tdTom_corr==TRUE,pool]), unique(meta[tdTom_corr==FALSE,pool])))>0 & length(unique(meta$pool))>1){ # No pools in experiment
            
      # Calculate norm factors
      sce_edger <- calcNormFactors(sce_edger)
      
      design <- model.matrix(~sce$tdTom_corr) # No pool in model
...
excl_genes = intersect(c(y_genes, x_genes, imprint, non_informative), rownames(sce)) # haem_genes, 'tomato-td'
...
# # Might just drop this altogether
...
# Save outputs
fwrite(de.results, file=sprintf('%s/DEresults_stage%s_tdTom_corr%s_%s.txt.gz', args$outdir, args$stage, args$tdTom_corr, args$method))
pdf(file=sprintf('%s/DEresults_stage%s_tdTom_corr%s_%s.pdf', args$outdir, args$stage, args$tdTom_corr, args$method), height=8, width=8)
    plots 
dev.off()

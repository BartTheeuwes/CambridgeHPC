...
getMappingScore <- function(mapping){
    ...
    celltypes_original_accrossK <- matrix(unlist(mapping$celltypes_originals.mapped),
                                  nrow=length(mapping$celltypes_originals.mapped[[1]]),
                                  ncol=length(mapping$celltypes_originals.mapped))
    ...
    out$celltypes_original.score <- NULL
    for (i in 1:nrow(celltypes_original_accrossK)){
        p <- max(table(celltypes_original_accrossK[i,]))
        index <- which(table(celltypes_original_accrossK[i,]) == p)
        p <- p/length(mapping$celltypes_original.mapped)
        out$celltypes_original.score <- c(out$celltypes_originals.score,p)
    }
    return(out)  
}
...
get_meta <- function(correct_atlas, atlas_meta, correct_map, map_meta, k_map = 10){
  ...
  celltypes_original.mapped <- apply(celltypes_original, 1, function(x) getmode(x, 1:length(x)))
  out <- lapply(1:length(celltype.mapped), function(x){
    list(cells.mapped     = k.mapped[x,],
         ...
         celltypes_original.mapped = celltypes_original.mapped[x],
         celltypes.mapped = celltypes[x,],
         stages.mapped    = stages[x,])
         celltypes_originals.mapped = celltypes_original[x,]
  })
...
mapWrap <- function(atlas_sce, atlas_meta, map_sce, map_meta, order = NULL, k = 30, npcs = 50, genes = NULL, return.list = FALSE){
  ...
  for (i in seq(from = 1, to = k)) {
    ...
    out$celltypes_originals.mapped[[i]] <- sapply(mapping, function(x) x$celltypes_originals.mapped[i])
  ...
  cto <- sapply(mapping, function(x) x$celltypes_original.mapped[1]); is.na(cm) <- lengths(cm) == 0

  out$mapping <- data.frame(
      cell            = names(mapping), 
      celltype.mapped = unlist(ct),
      celltypes_original.mapped = unlist(cto),

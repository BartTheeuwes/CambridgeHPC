...
# args$lsi <- file.path(io$basedir,"results_new/atac/archR/dimensionality_reduction/lsi_PeakMatrix_nfeatures5000_ndims30.txt.gz")
# args$umap <- file.path(io$basedir,"results_new/atac/archR/dimensionality_reduction/umap_PeakMatrix_nfeatures5000_ndims30.txt.gz")
# args$input_celltype_column <- "celltype.mapped_mnn"
# args$output_celltype_column <- "celltype.predicted"
# args$k <- 25
# args$outdir <- file.path(io$basedir,"results_new/atac/archR/celltype_assignment")
## END TEST ##
...
if (sum(is.na(sample_metadata.filt$celltype))==0) {
...
cat("Loading LSI and UMAP representations...\n")

# LSI
lsi.dt <- fread(args$lsi) 
missing.cells <- lsi.dt[!cell%in%sample_metadata$cell,cell]
if (length(missing.cells)>0) {
  cat(sprintf("\n%d cells are found in the LSI file but missing in the metadata file:\n%s",length(missing.cells), paste(missing.cells, collapse="\n")))
}
lsi.dt <- lsi.dt[cell%in%sample_metadata$cell]
lsi.mtx <- lsi.dt %>% matrix.please

# UMAP
umap.dt <- fread(args$umap)
missing.cells <- umap.dt[!cell%in%sample_metadata$cell,cell]
if (length(missing.cells)>0) {
  cat(sprintf("\n%d cells are found in the UMAP file but missing in the metadata file:\n%s",length(missing.cells), paste(missing.cells, collapse="\n")))
}
umap.dt <- umap.dt[cell%in%sample_metadata$cell]

# Filter cells from the sample metadata 
stopifnot(sort(umap.dt$cell)==sort(lsi.dt$cell))
sample_metadata.filt <- sample_metadata.filt[cell%in%umap.dt$cell]
...
png(file.path(args$outdir,"celltype_assignment.png"), width=1400, height=500)
print(p)
dev.off()

...
markersPeaks <- getMarkerFeatures(
  ...
  useGroups = c("Erythroid1","Erythroid2","Erythroid3"),
  bgdGroups = c("Endothelium")
)
...
markersPeaks.dt.filt[,sig:=FDR<0.1 & abs(MeanDiff)>0.25] %>% 
  .[,sign:="Up in Erythroid"] %>% .[Log2FC<0,sign:=c("Up in Endothelium")] 
...
average.peak.celltype.se <- readRDS(io$archR.pseudobulk.peaks)
...
order.celltypes <- to.plot %>% .[,.(value=median(value)),by=c("celltype","sign")] %>% 
  dcast(celltype~sign) %>% 
  .[,diff:=`Up in Erythroid` - `Up in Endothelium`] %>%

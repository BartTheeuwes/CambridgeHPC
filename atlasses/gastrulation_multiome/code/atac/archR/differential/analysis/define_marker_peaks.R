...
# I/O
io$archR.diff.dir <- file.path(io$basedir,"results_new/atac/archR/differential/PeakMatrix")
...
dt <- opts$celltypes %>% map(function(i) { opts$celltypes %>% map(function(j) {
  file <- sprintf("%s/PeakMatrix_%s_vs_%s.txt.gz", io$archR.diff.dir,i,j)
  if (file.exists(file)) {
...
fwrite(markers_peaks.dt, file.path(io$outdir,"marker_peaks.txt.gz"))

...
io$marker.genes <- paste0(io$basedir,"/results/atac/archR/marker_peaks/genes/markers_all.tsv.gz")
...
for (i in unique(dt$celltype)) {
  ...
  negative_hits <- to.plot[sig==TRUE & Log2FC<0,gene]
  positive_hits <- to.plot[sig==TRUE & Log2FC>0,gene]
  all <- nrow(to.plot)
  ...
  png(sprintf("%s/volcano_plots/volcano_marker_genes_%s.png",io$outdir,i), width = 800, height = 500)
  print(p)
  dev.off()
}

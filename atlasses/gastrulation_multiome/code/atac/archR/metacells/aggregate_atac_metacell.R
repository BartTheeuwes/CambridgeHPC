...
# args$matrices <- c("GeneScoreMatrix_TSS") # c("PeakMatrix", "GeneScoreMatrix_distal", "GeneScoreMatrix_TSS")
...
for (i in args$matrices) {
  ...
  # summarise
  atac_pseudobulk.se <- getGroupSE(ArchRProject, groupBy = "metacell", useMatrix = i, divideN = FALSE)
  ...
  # colData(atac_pseudobulk.se) <- metacell_metadata.dt %>% tibble::column_to_rownames("metacell") %>% DataFrame
  
  # save
  saveRDS(atac_pseudobulk.se, file.path(args$outdir,sprintf("%s_summarized_experiment_metacells.rds",i)))
}
...
metacell_metadata.dt <- sample_metadata %>%
  .[cell%in%colnames(atac_pseudobulk.se)] %>% setkey(cell) %>% .[colnames(atac_pseudobulk.se)] %>%
  .[,c("metacell", "cell", "sample", "stage", "genotype",  "celltype", "closest.cell")] %>%
  merge(cell2metacell.dt[,.(ncells=.N),by="metacell"],by="metacell")

# Add QC stats
tmp <- data.table(
  metacell = colnames(atac_pseudobulk.se), 
  nFrags_atac = colSums(assay(atac_pseudobulk.se))
)

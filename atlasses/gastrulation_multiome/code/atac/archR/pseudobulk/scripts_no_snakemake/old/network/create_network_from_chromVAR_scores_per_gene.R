...
# I/O
io$outdir <- paste0(io$basedir,"/results/atac/archR/chromvar/pseudobulk/network")
...
chromvar.cor <- cor(chromvar.mtx)

# Parse correlation results
diag(chromvar.cor) <- 0
chromvar.cor[chromvar.cor<0.50] <- 0
...
pdf(paste0(io$outdir,"/TF_network_coloured_by_celltype_nolabelled.pdf"), width=10, height=8)
print(p)
dev.off()

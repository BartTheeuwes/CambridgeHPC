...
# } else if(grepl("bt392", Sys.info()['effective_user'])) {
...
io$TFs_file <- "/rds/project/rds-SDzz0CATGms/users/bt392/atlasses/TFs.txt"
io$archR.directory <- file.path(io$basedir,"processed/atac/archR")
# } else {
...
io$rna.celltype_tf_markers <- file.path(io$basedir,"results/rna/differential/metacells/celltype/TFs/marker_TFs_all.txt.gz")
...
io$archR.GeneScoreMatrix.pseudobulk <- file.path(io$basedir,"processed/atac/archR/Matrices/GeneScoreMatrix_TSS_summarized_experiment.rds")
...
io$pijuansala.seurat <- file.path(io$pijuansala.basedir,"data/processed/seurat.rds")
io$pijuansala.fragments <- file.path(io$pijuansala.basedir,"data/fragments.tsv.gz")
io$pijuansala.fragments <- file.path(io$pijuansala.basedir,"data/fragments.tsv.gz")

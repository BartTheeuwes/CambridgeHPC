...
# )
# # which celltypes to perform diffacc
# opts$celltype1 <- "Forebrain_Midbrain_Hindbrain"
# opts$celltype2 <- "Surface_ectoderm"
...
meta[, stage := strsplit(sample, "_") %>% map_chr(1)]
meta[, stage_celltype := paste0(stage, "_", celltype.predicted)]
...
# then compute mean accessibility in these sites in each cell

up <- da_peaks[p_val_adj < 0.05 & avg_logFC > 0.5, locus]
down  <- da_peaks[p_val_adj < 0.05 & avg_logFC < -0.5, locus]

sub <- signac[c(up,down),] 
dt <- as.data.table(sub@assays$peaks@data, keep.rownames = "id") %>% 
  melt(id.vars = "id", value.name = "acc", variable.name = "cell") %>% 
  merge(meta[, .(cell, celltype.predicted)], by = "cell") %>% 
  .[id %in% up, type := "up"] %>% 
  .[id %in% down, type := "down"] %>% 
  .[, .(acc = mean(acc)), .(type, celltype.predicted, id)] %>% 
  .[, celltype := factor(celltype.predicted, levels = opts$celltypes)] %>% 
  .[type == "up", type := paste(opts$celltype1, "hyper-accessible sites")] %>% 
  .[type == "down", type := paste(opts$celltype2, "hyper-accessible sites")]
...
p <- ggplot(dt, aes(celltype, acc, fill = celltype)) +
  ...
  ylab("Mean accessiblity accross cells") +
  facet_wrap(~type)

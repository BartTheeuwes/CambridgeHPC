...
io$differential_results <- file.path(io$basedir,"results_new/rna/differential/TFs")
io$outdir <- file.path(io$basedir,"results_new/rna/differential/TFs/marker_TFs"); dir.create(io$outdir, showWarnings = F)
...
foo <- dt[,.(score=sum(direction=="up" & sig==TRUE,na.rm=T)), by=c("celltypeA","gene")] %>% setnames("celltypeA","celltype")
bar <- dt[,.(score=sum(direction=="down" & sig==TRUE,na.rm=T)), by=c("celltypeB","gene")] %>% setnames("celltypeB","celltype")
...
fwrite(markers_genes.dt, file.path(io$outdir,"marker_TFs_upregulated_all.txt.gz"))
...
fwrite(markers_genes_filt.dt, file.path(io$outdir,"marker_TFs_upregulated_filtered.txt.gz"))

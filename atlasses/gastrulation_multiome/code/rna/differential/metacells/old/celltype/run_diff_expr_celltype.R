...
io$script <- here::here("rna/differential/metacells/differential.R")
  ...
  for (j in i:length(celltypes.to.use)) {
    if (i!=j) {
      ...
      if (!file.exists(outfile)) {
        ...
        cmd <- sprintf("%s 'Rscript %s --metadata %s --sce %s --groupA %s --groupB %s --group_variable %s --outfile %s'", 
          lsf, io$script, args$metadata, args$sce, groupA, groupB, args$group_variable, outfile)

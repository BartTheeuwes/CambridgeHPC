...
io$outdir <- paste0(io$basedir,"/results/rna/marker_genes")#; dir.create(io$outdir, showWarnings = F)
...
############################################
## Load pseudobulk RNA intronic vs exonic ##
############################################
...
assays(sce.pseudobulk)
# Subset features
# sce.pseudobulk <- sce.pseudobulk[rownames(sce.pseudobulk) %in% unique(marker_genes.dt$gene),]
# sce.pseudobulk <- sce.pseudobulk[!rownames(sce.pseudobulk) %in% unique(marker_genes.dt$gene),]
# sce.pseudobulk <- sce.pseudobulk[grep("Olfr",rownames(sce.pseudobulk)),]

# Prepare data.table
rna_unspliced.dt <- assay(sce.pseudobulk,"unspliced_log") %>%
  as.data.table(keep.rownames="gene") %>% 
  melt(id.vars="gene", variable.name="celltype", value.name="unspliced_expr")

rna_spliced.dt <- assay(sce.pseudobulk,"spliced_log") %>%
  as.data.table(keep.rownames="gene") %>% 
...
celltype.to.plot <- "Epiblast"

to.plot <- rna_markers.dt[class==sprintf("%s markers",celltype.to.plot)] %>%
  .[,ratio:=unspliced_expr/spliced_expr] %>%
  .[,.(ratio=mean(ratio)), by=c("class","celltype")]

to.plot2 <- to.plot %>%
  merge(
    celltype_distance.dt[celltypeA==celltype.to.plot] %>% .[,celltypeA:=NULL] %>% setnames("celltypeB","celltype"), by="celltype"
  )

p <- ggscatter(to.plot2, x="distance", y="ratio", fill="celltype", size=4, shape=21,
               add="reg.line", add.params = list(color="black", fill="lightgray"), conf.int=TRUE) +
  stat_cor(method = "pearson") +
  facet_wrap(~class, scales="fixed") +
  scale_fill_manual(values=opts$celltype.colors) +
  labs(x="Distance between celltypes", y="Ratio unspliced/spliced counts") +
  theme(
    strip.text = element_text(size=rel(0.85)),
    axis.text = element_text(color="black", size=rel(0.75)),

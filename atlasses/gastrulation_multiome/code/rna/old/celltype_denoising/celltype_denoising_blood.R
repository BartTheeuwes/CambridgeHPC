...
# PCA
sce_filt <- runPCA(sce_filt, ncomponents = 15, ntop = 1000)
...
# UMAP
set.seed(42)
sce_filt <- runUMAP(sce_filt, dimred="PCA", n_neighbors = 15, min_dist = 0.15)

###########################
## Plot before denoising ##
###########################

plotUMAP(sce_filt, colour_by="sample")
...
plotPCA(sce_filt, colour_by="celltype.mapped") +
  scale_color_manual(values=opts$celltype.colors) + 
  theme(
    legend.position = "none"
  )

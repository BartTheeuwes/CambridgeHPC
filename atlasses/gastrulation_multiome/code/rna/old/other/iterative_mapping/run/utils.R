...
recursive.fn <- function(sce_query, sce_atlas, dist, npcs = 5, k = 25, ...) {
  
  celltypes.to.loop <- sce_query$celltype_mapped %>% unique %>% .[grep("%",.)]
  
  dt_list <- list()
  for (i in celltypes.to.loop) {
    ids <- which(sce_query$celltype_mapped==i)
    ...
    sce_query.sub <- sce_query[,ids]
    sce_atlas.sub <- sce_atlas[,sce_atlas$celltype %in% foo]
    
    # Run cell type assignment prediction
    dt_list[[i]] <- mapping.fn(sce_query.sub, sce_atlas.sub, h.sub, npcs = npcs, k = k, ...)
  }
...
mapping.fn <- function(sce_query.sub, sce_atlas.sub, h.sub, npcs = 5, k = 25, ...) {
  
  # Cut the tree into two groups
  cut <- cutree(h.sub, k=2)
  
  # Define groups
  groupA <- names(which(cut==1))
  groupB <- names(which(cut==2))
  groups.parsed <- c(paste(groupB,collapse="%"),paste(groupA,collapse="%"))
  sce_atlas.sub$celltype <- groups.parsed[as.numeric(sce_atlas.sub$celltype%in%groupA)+1]
  
  # Run differential expression between groupA and groupB cells
  # diff <- differential_expression(sce_atlas.sub, groupA, groupB)
  diff <- differential_expression(sce_atlas.sub, groups.parsed[1], groups.parsed[2])
  
  # Select genes
  markers.groupA <- diff[padj_fdr<0.10 & logFC>1,ens_id]
  markers.groupB <- diff[padj_fdr<0.10 & logFC<(-1),ens_id]
  genes <- c(markers.groupA,markers.groupB)
  
  # Joint normalisation
  sce.all <- joint.normalisation(sce_query.sub, sce_atlas.sub, ...)
  
  # Run MNN
  dt <- mnn.fn(sce.all, sce_query.sub, sce_atlas.sub, genes = genes, npcs = npcs, k = k)
...
joint.normalisation <- function(sce_query, sce_atlas, cosineNorm = FALSE) {
  ...
  if (isTRUE(cosineNorm)) {
    ...
    assay(sce_atlas, "cosineNorm") <- cosineNorm(assay(sce_atlas, "logcounts"))
    assay(sce_query, "cosineNorm") <- cosineNorm(assay(sce_query, "logcounts"))
    
    # Concatenate
    sce.all <- SingleCellExperiment(
      list(cosineNorm=cbind(assay(sce_atlas,"cosineNorm"), assay(sce_query,"cosineNorm")))
    )
...
mnn.fn <- function(sce.all, sce_query, sce_atlas, npcs, k, genes = NULL) {
  ...
  # if (is.null(genes)) {
  #   # genes <- getHVGs(sce_atlas, block=as.factor(sce_atlas$sample), assay.type="logcounts")
  #   genes <- getHVGs(sce.all, block=block, assay.type="logcounts")
  #   # genes <- rownames(sce_atlas)
  # }
  ...
  atlas_pca <- pca_all[1:ncol(sce_atlas),]
  query_pca <- pca_all[-(1:ncol(sce_atlas)),]

  # (Optional) Batch effect correction for the atlas
  # atlas_meta <- colData(sce_atlas)[,c("stage","sample")] %>% as.data.frame
  # stages <- sort(unique(sce_atlas$stage))
  ...
  # get metadata
  meta_query <- as.data.frame(colData(sce_query)) %>% .[,c("cell"),drop=F]
  meta_atlas <- as.data.frame(colData(sce_atlas)) %>% .[,c("cell","celltype","stage")]
  mapping <- get_meta(
...
differential_expression <- function(sce_atlas, groupA, groupB) {
  
  
  # Filter genes by detection rate per group
  # opts$min_detection_rate_per_group <- 0.25
  cdr_A <- rowMeans(logcounts(sce_atlas[,sce_atlas$celltype%in%groupA])>0) >= 0.25
  cdr_B <- rowMeans(logcounts(sce_atlas[,sce_atlas$celltype%in%groupB])>0) >= 0.25
  sce_atlas <- sce_atlas[cdr_A|cdr_B,]
  
  # Convert SCE to DGEList
  sce_edger <- scran::convertTo(sce_atlas, type="edgeR")
  
  # Define design matrix
  cdr <- colMeans(logcounts(sce_atlas)>0)
  design <- model.matrix(~cdr+sce_atlas$celltype)

ComputeDORC <- function( 
  ...
  if (length(coef.vec) == 0) {
    if (verbose) {
      message("No correlationificant links found")
    }
    return(object)
  }
  peak.key <- seq_along(
  ...
  links$correlation[links$score>0]='positive'
  links$correlation[links$score<0]='negative'
  return(links)
}

...
# just for me
load("/home/lijingyu/git/stimATAC_analyses_code/data/mm10_refSeq.RData")
...
runGenePeakcorr <- function(ATAC.se, # SummarizedExperiment object of scATAC data
  ...
  stopifnot(inherits(ATAC.se,"RangedSummarizedExperiment")|inherits(ATAC.se,"SummarizedExperiment"))
  stopifnot(inherits(RNAmat,c("Matrix","matrix")))
  
  if(!all.equal(colnames(ATAC.se),colnames(RNAmat)))
  ...
  ATACmat <- as(assay(ATAC.se),'dgCMatrix') # Rownames preserved

...
runFigR <- function(ATAC.se, # SE of scATAC peak counts. Needed for chromVAR bg peaks etc.
  ...
  if (is.null(rowData(ATAC.se)$bias)) {
    ...
    if(!is(ATAC.se,'RangedSummarizedExperiment')){
      ATAC.se <- chromVAR::addGCBias(ATAC.se,peakRanges,genome=myGenome)
    }else{
  ...
  if(!is(ATAC.se,'RangedSummarizedExperiment')){
    peak.dt <- ATAC.se@elementMetadata@listData
    peakRanges <- GRanges(seqnames =as.character(peak.dt$seqnames),ranges = IRanges(peak.dt$start,peak.dt$end))
  }
  ATAC.se <- SummarizedExperiment(assays=assay(ATAC.se),
                       rowRanges=peakRanges, colData=ATAC.se@colData)
  ...
  mZtest.list <- foreach(g=dorcGenes,
                         ...
                         .packages = c( "dplyr","Matrix")) %dopar%   {
                           ...
                           corr.r <- cor(dorcMat[g,],t(as.matrix(rnaMat[as.character(mZ$Motif),])),method = "spearman")
  ...
  TFenrich.d <- TFenrich.d %>% mutate("Score"=sign(Corr)*as.numeric(-log10(1-(1-Enrichment.P)*(1-Corr.P))))
...
plotDrivers <- function(figR.d,
  ...
  gScatter <- d %>%  ggplot(aes(x=Corr.log10P,y=Enrichment.log10P,color=isSig,label=Label)) + 
    ...
    labs( x="-sign*Corr.log10P",y="-sign(Enrichment.Z)*Enrichment.log10P",title=marker) + 
...
build_GRN_matrix <- function(virtual_chip.mtx,DORC.filt){
  peaks_dorc <- unique(DORC.filt$Peak)
  virtual_chip.mtx <- virtual_chip.mtx[rownames(virtual_chip.mtx)%in%peaks_dorc,]
  
  # Find target genes for each TF
  TF_target_list <- lapply(colnames(virtual_chip.mtx),TF_target)
  names(TF_target_list) <- colnames(virtual_chip.mtx)
  
  # From list to TF_target_matrix
  TF_target_list <- TF_target_list[lengths(TF_target_list)>0]
  TF_target_df <- sapply(TF_target_list, `length<-`, max(lengths(TF_target_list)))
  TF_target_pair <- stack(TF_target_df)
  TF_target_pair <- TF_target_pair[!is.na(TF_target_pair$value),] 
  colnames(TF_target_pair) <- c('value','TF','target')
  TF_target_pair$value <- 1
  TF_target_matrix <- spread(as.data.frame(TF_target_pair), TF, value)
  colnames(TF_target_matrix) <- firstup(tolower(colnames(TF_target_matrix)))
  TF_target_matrix[is.na(TF_target_matrix)] = 0
  return(TF_target_matrix)
}


build_GRN_df <- function(virtual_chip.mtx,DORC.filt){
  peaks_dorc <- unique(DORC.filt$Peak)
  virtual_chip.mtx <- virtual_chip.mtx[rownames(virtual_chip.mtx)%in%peaks_dorc,]
  ...
  TF_target_pair <- TF_target_pair[!is.na(TF_target_pair$value),] 
  colnames(TF_target_pair) <- c('value','TF','target')
  TF_target_pair <- TF_target_pair[,c('TF','target')]
  TF_target_pair$TF <- firstup(tolower(as.character(TF_target_pair$TF)))
  return(TF_target_pair)
}
...
peak_target <- function(peak){
  DORC.filt[DORC.filt$Peak==peak,]$Gene
}

TF_target <- function(TF){
  target <- c()
  peaks_list <- virtual_chip.mtx[,TF] 
  peaks_list <- names(peaks_list[peaks_list>0.2])
  target_list <- lapply(peaks_list,peak_target)
  target_list <- unique(unlist(target_list))
}

KO_simulate <- function(gene_KO,GRN_coef.mtx,exprMat,n_propagation){
  delta_simulated <- t(exprMat)
  delta_simulated[delta_simulated>0] <- 0
  delta_simulated[,gene_KO] <- - exprMat[gene_KO,]
  for (i in 1:n_propagation){
    delta_simulated = delta_simulated %*% GRN_coef.mtx
    delta_simulated[,gene_KO] = - exprMat[gene_KO,]
    #Gene expression can not be negative. adjust delta values to make sure that gene expressions are not negative values.
    exprMat_tmp = t(exprMat) + delta_simulated
    exprMat_tmp[exprMat_tmp<0] = 0
    delta_simulated = exprMat_tmp - t(exprMat)}
  
  return(delta_simulated)
}

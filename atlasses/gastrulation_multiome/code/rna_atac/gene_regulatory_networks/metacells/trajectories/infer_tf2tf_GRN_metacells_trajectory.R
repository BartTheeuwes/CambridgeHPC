...
args$rna_metacells.sce <- file.path(io$basedir, 'results/rna/metacells/trajectories/nmp/SingleCellExperiment_metacells.rds')
args$rna_pseudobulk.sce <- file.path(io$basedir,"results/rna/pseudobulk/celltype/SingleCellExperiment_pseudobulk.rds")
args$trajectory <- file.path(io$basedir,'results/rna/trajectories/nmp/nmp_trajectory.txt.gz')
args$grn_coef <- file.path(io$basedir,'results/rna_atac/gene_regulatory_networks/metacells/trajectories/nmp/global_chip_GRN_coef.txt.gz')
args$min_coef <- 0.25
args$trajectory <- "nmp"
args$trajectory_file <- "/Users/argelagr/data/gastrulation_multiome_10x/results/rna/trajectories/nmp/nmp_trajectory.txt.gz"
args$min_tf_marker_strength <- 0.75
# args$min_gene_marker_strength <- 0.25
args$outdir <-  file.path(io$basedir,"results/rna_atac/gene_regulatory_networks/metacells/trajectories/nmp/test")
## END TEST
...
marker_TFs_filt.dt <- marker_TFs_all.dt %>% 
  .[score>=args$min_tf_marker_strength]
...
print(unique(GRN_coef.dt$tf))
print(unique(GRN_coef.dt$gene))
...
# TFs <- Reduce("intersect",list(unique(GRN_coef.dt$tf),toupper(rownames(rna_metacells.sce))))
# TFs <- unique(c(GRN_coef.dt$tf,GRN_coef.dt$gene))

# Make sure we keep T/Sox2
# GRN_coef.dt[tf=="T" | gene=="T"]
# GRN_coef.dt[tf=="SOX2" | gene=="SOX2"]
...
pdf(file.path(args$outdir,"global_network.pdf"), width = 10, height = 8)
print(p)
dev.off()
...
for (i in celltypes.to.plot) {
  ...
  pdf(sprintf("%s/%s_network.pdf",args$outdir,i), width = 10, height = 7)
  print(p2)
  dev.off()
...
pdf(file.path(args$outdir,"global_network_highlight_T.pdf"), width = 13, height = 6.5)
print(p)
dev.off()

...
marker_TFs.dt <- fread(io$rna.atlas.marker_TFs.up) %>% .[celltype%in%args$celltype]

marker_genes.dt <- fread(io$rna.atlas.marker_genes.up) %>%
  .[!gene%in%unique(marker_TFs.dt$gene) & celltype%in%args$celltype]
...
# Define genes
opts$genes <- marker_genes.dt$gene[marker_genes.dt$gene%in%rownames(sce.pseudobulk)]

# Filter genes manually
sce.pseudobulk.filt <- sce.pseudobulk[opts$genes,]

###############################
## Load TF2gene correlations ##
###############################

tf2gene_cor.se <- readRDS(io$tf2gene_cor.se)[opts$TFs,opts$genes]
...
virtual_chip.dt <- opts$TFs %>% map(function(i) {
  chip.file <- sprintf("%s/%s.bed.gz",io$virtual_chip.dir,i)
  if (file.exists(chip.file)) {
    virtual_chip.dt <- fread(chip.file) %>%
      setnames(c("chr","start","end","score")) %>%
      .[,peak:=sprintf("%s:%s-%s",chr,start,end)] %>%
      .[peak%in%unique(peak2gene.dt$peak) & score>=0.10] %>%
...
######################################################
## Use ChIP-seq data to filter TF-gene associations ##
######################################################

opts$cor.threshold <- 0.30

corr.mtx <- as.matrix(assay(tf2gene_cor.se,"cor"))
for (i in opts$TFs) {

  # Select target peaks
  target_peaks_i <- virtual_chip.dt[tf==i,peak]

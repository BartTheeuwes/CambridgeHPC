...
tf2gene.mtx <- readRDS("/Users/ricard/data/gastrulation_multiome_10x/results/rna_atac/gene_regulatory_networks/pseudobulk/per_TF/MIXL1/corr_matrix_stringent.rds")
...
diff_rna.dt <- fread("/Users/ricard/data/10x_gastrulation_Mixl1_Chimera/results/differential/pseudobulk/DE_pseudobulk.txt.gz")
...
tmp <- diff_rna.dt[gene%in%c("Mixl1",names(tf2gene_filt.mtx))] %>% setkey(gene) %>% .[str_to_title(names(V(net)))]

...
#   .[gene%in%str_to_title(colnames(virtual_chip.mtx)) & dist<=args$distance] %>%
#   .[,gene:=toupper(gene)] %>%
...
rna_genes.sce <- rna.sce[!rownames(rna.sce)%in%str_to_title(colnames(virtual_chip.mtx)),]
...
# Subset genes
rna_genes_pseudobulk.sce <- rna_pseudobulk.sce[rownames(rna_genes.sce),]

rm(rna_pseudobulk.sce)
dim(rna_tf_pseudobulk.sce)
dim(rna_genes_pseudobulk.sce)
...
# Filter TFs that have predicted ChIP-seq annotations
TFs <- intersect(rownames(rna_tf.sce),colnames(virtual_chip.mtx))
rna_tf.sce <- rna_tf.sce[TFs,]
rna_tf_pseudobulk.sce <- rna_tf_pseudobulk.sce[TFs,]
virtual_chip.mtx <- virtual_chip.mtx[,TFs]

# Filter genes
peak2gene.dt <- peak2gene.dt[gene%in%rownames(rna_genes.sce)]

# Filter peaks
peaks <- intersect(rownames(virtual_chip.mtx), unique(peak2gene.dt$peak))
virtual_chip.mtx <- virtual_chip.mtx[peaks,]
peak2gene.dt <- peak2gene.dt[peak%in%peaks]

# Filter genes by variability
...
peaks <- intersect(unique(peak2gene.dt$peak),rownames(tf2peak_cor.se))
...
# TO-DO: USE THE IN SILICO CHIP-SEQ LIBRARY TO BUILD GRN

opts$min.chip.threshold <- 0.30
opts$cor.threshold <- 0.50

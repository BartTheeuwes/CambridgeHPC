...
io$MOFAobject <- paste0(io$basedir, "/results/rna_atac/MOFAobject/NMPs/mofa_model.rds")
...
plot_factor(MOFAobject, factors = c(5), color_by = "celltype.mapped", group_by = "celltype.mapped", add_violin = T, add_boxplot = T, add_dots = F, dodge=T, legend=F) +
...
plot_factors(MOFAobject, factors = c(1,5), color_by = "mtFraction_RNA")

plot_factors(MOFAobject, factors = c(1,5), color_by = "celltype.mapped") +
  scale_fill_manual(values=opts$celltype.colors)

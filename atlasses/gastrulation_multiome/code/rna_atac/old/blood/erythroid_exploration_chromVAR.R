...
# I/O
io$outdir <- paste0(io$basedir,"/results/rna_atac/blood/chromvar")
...
for (i in genes.to.plot) {
  ...
  to.plot <- chromvar_rna_dt[gene==i] %>% 
    ...
    melt(id.vars=c("cell","rank_V1","celltype"), measure.vars=c("chromvar_zscore","expr"), variable.name="modality")# %>%
    # .[,value_scaled:=value/max(value),by="modality"]
  # .[,value_scaled:=(value-min(value))/(max(value)-min(value)), by="modality"]
    
  p1 <- ggplot(to.plot, aes(x=rank_V1, y=value)) +
...
to.plot <- chromvar_rna_dt[gene%in%genes.to.test,] %>%

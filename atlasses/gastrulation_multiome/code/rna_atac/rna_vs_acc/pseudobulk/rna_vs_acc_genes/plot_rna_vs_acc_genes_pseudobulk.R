...
here::i_am("rna_atac/rna_vs_acc/pseudobulk/rna_vs_acc_genes/plot_rna_vs_acc_genes_pseudobulk.R")
...
# args$gene_score_matrix <- file.path(io$basedir,"results_new/atac/archR/pseudobulk/celltype.mapped_mnn/pseudobulk_GeneScoreMatrix_TSS_summarized_experiment.rds") # io$archR.pseudobulk.GeneMatrix.se
# args$outdir <- file.path(io$basedir,"results_new/rna_atac/rna_vs_acc/pseudobulk/per_gene")
## END TEST ##
...
# rna_atac.dt <- rna_atac.dt[grep("Rik",gene,invert = T)]
# rna_atac.dt <- rna_atac.dt[gene!="Xist"]
# rna_atac.dt <- rna_atac.dt[!grepl("mt-",gene)]
# rna_atac.dt <- rna_atac.dt[!grepl("Rps|Rpl",gene)]
# rna_atac.dt <- rna_atac.dt[!grepl("Rik",gene)]
# rna_atac.dt <- rna_atac.dt[!gene%in%fread(io$gene_metadata)[chr=="chrY",symbol]]
...
# Save
fwrite(cor.dt, file.path(args$outdir,"cor_rna_vs_acc_per_gene_pseudobulk.txt.gz"), sep="\t", quote=F)
...
for (i in genes.to.plot) {
  outfile <- file.path(args$outdir,sprintf("individual_genes/%s_rna_vs_acc_pseudobulk.png",i))
  if (!file.exists(outfile)) {
    ...
    # p <- (p.scatter+p.barplot)/(p.paga.expr+p.paga.acc) + plot_layout(heights=c(1,1))
    ...
    png(outfile, width = 1500, height = 400)

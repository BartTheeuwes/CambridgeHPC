...
# I/O
io$outdir <- paste0(io$basedir,"/results/rna_atac/rna_vs_chromvar/trajectories/ectoderm_trajectory")
...
io$pseudotime <- "/Users/ricard/data/gastrulation_multiome_10x/results/rna/trajectories/ectoderm_trajectory/ectoderm_trajectory.txt.gz"
...
io$hvgs <- "/Users/ricard/data/gastrulation_multiome_10x/results/rna/trajectories/ectoderm_trajectory/hvgs.rds"
...
fwrite(chromvar_rna_dt, sprintf("%s/chromvar_rna_ectoderm.txt.gz",io$outdir), quote=F, sep="\t", na="NA")
...
for (i in genes.to.plot) {
  ...
  pdf(sprintf("%s/individual_genes/%s_rna_chromvar_vs_pseudotime_ectoderm.pdf",io$outdir,i), width=8, height=6)
  print(p)
  dev.off()
}
...
directionality.dt <- chromvar_rna_dt %>%
  ...
  .[,diff:=E8.5_Forebrain_Midbrain_Hindbrain-E7.5_Epiblast] %>% 
...
fwrite(cor.dt, sprintf("%s/correlation_results_ectoderm.txt.gz",io$outdir), quote=F, sep="\t", na="NA")

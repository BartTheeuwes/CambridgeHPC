...
# I/O
io$outdir <- paste0(io$basedir,"/results/rna_atac/rna_vs_chromvar/trajectories/nmp_trajectory")
...
io$pseudotime <- "/Users/ricard/data/gastrulation_multiome_10x/results/rna/trajectories/NMP_trajectory/nmp_trajectory.txt.gz"
...
io$hvgs <- "/Users/ricard/data/gastrulation_multiome_10x/results/rna/trajectories/nmp_trajectory/hvgs.rds"
...
fwrite(chromvar_rna_dt, sprintf("%s/chromvar_rna_nmp.txt.gz",io$outdir), quote=F, sep="\t", na="NA")
# chromvar_rna_dt <- fread(sprintf("%s/chromvar_rna_nmp.txt.gz",io$outdir))
...
for (i in genes.to.plot) {
  ...
  pdf(sprintf("%s/individual_genes/%s_rna_chromvar_vs_pseudotime_nmp.pdf",io$outdir,i), width=8, height=6)
  print(p)
  dev.off()
}
...
directionality.dt <- chromvar_rna_dt %>%
  .[,.(chromvar_zscore=mean(chromvar_zscore)),by=c("gene","motif","celltype.mapped")] %>% 
  dcast(gene+motif~celltype.mapped, value.var=c("chromvar_zscore")) %>%
  .[,diff:=Somitic_mesoderm-Spinal_cord] %>% 
...
fwrite(cor.dt, sprintf("%s/correlation_results_nmp.txt.gz",io$outdir), quote=F, sep="\t", na="NA")
...
####################################################
## Coexpression of T and Sox2 motif accessibility ##
####################################################
...
to.plot <- chromvar_rna_dt %>%
  ...
  setnames("celltype.mapped","celltype") %>%
  dcast(cell+celltype+V1~gene, value.var="chromvar_zscore") %>%
  .[,prod:=sqrt(`T`*Sox2)]


p1 <- ggplot(to.plot, aes(x=V1, y=prod)) +
  geom_point(aes(fill=celltype), size=1.25, shape=21, stroke=0.1) +
  stat_smooth(method="loess", color="black", alpha=0.75, span=0.5) +
  geom_rug(aes(color=celltype), sides="b") +
  scale_color_manual(values=opts$celltype.colors) +
  ...
  # scale_fill_manual(values=opts$celltype.colors) +
  # scale_fill_brewer(palette="Dark2") +
  labs(x="Pseudotime", y="T * Sox2 motif accessibility") +
  theme_classic() +
  theme(
    axis.text.x = element_blank(),
    axis.ticks.x = element_blank(),
...
p2 <- ggboxplot(to.plot, x="celltype", y="prod", fill="celltype", outlier.shape=NA) +
  scale_fill_manual(values=opts$celltype.colors) +
  labs(x="", y="") +
  theme_classic() +
  guides(x = guide_axis(angle = 90)) +

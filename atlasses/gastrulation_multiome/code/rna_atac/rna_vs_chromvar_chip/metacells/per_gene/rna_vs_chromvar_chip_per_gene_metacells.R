here::i_am("rna_atac/rna_vs_chromvar_chip/metacells/per_gene/rna_vs_chromvar_chip_per_gene_metacells.R")
...
p$add_argument('--metadata',  type="character",              help='Metacell metadata') 
...
args$rna_sce <- file.path(io$basedir,"results/rna/metacells/all_cells/SingleCellExperiment_TFs_metacells.rds")
args$atac_chromvar_chip <- file.path(io$basedir,sprintf("results/atac/archR/chromvar_chip/metacells/chromVAR_chip_%s_archr.rds",args$motif_annotation))
args$outdir <- file.path(io$basedir,"results/rna_atac/rna_vs_chromvar_chip/metacells/per_gene")
## END TEST ##
...
rna_tf.dt <- logcounts(rna.sce) %>%
  as.data.table(keep.rownames = T) %>%
  setnames("rn","gene") %>%
  data.table::melt(id.vars="gene", variable.name="metacell", value.name="expr")
...
rna_chromvar.dt <- merge(
  ...
  atac_chromvar.dt,
  by = c("metacell","gene")
) %>% merge(metacells_metadata.dt[,c("metacell","celltype","sample","stage")], by = "metacell")

length(unique(rna_chromvar.dt$gene))

# Filter out cell types with very few metacells
celltypes.to.use <- rna_chromvar.dt[,length(unique(metacell)),by="celltype"] %>% .[V1>=5,celltype]
rna_chromvar.dt <- rna_chromvar.dt[celltype%in%celltypes.to.use]
...
# Save
fwrite(cor.dt, file.path(args$outdir,sprintf("cor_rna_vs_chromvar_chip_%s_per_gene.txt.gz",args$motif_annotation)), sep="\t", quote=F)
...
pdf(file.path(args$outdir,sprintf("volcano_cor_rna_vs_chromvar_chip_%s_per_gene_metacells.pdf",args$motif_annotation)), width = 7, height = 5)
print(p)
dev.off()
...
for (i in genes.to.plot) {
  ...
  to.plot2 <- to.plot %>% melt(id.vars=c("metacell","celltype","gene"), measure.vars=c("expr","chromvar_zscore"))

  p1 <- ggscatter(to.plot, x="expr", y="chromvar_zscore", fill="celltype", size=2, shape=21, 
  ...
  p2 <- ggboxplot(to.plot2, x="celltype", y="value", fill="celltype", outlier.shape=NA) +
    facet_wrap(~variable, nrow=2, scales="free_y",  labeller = as_labeller(facet.labels)) +
    scale_fill_manual(values=opts$celltype.colors) +
    stat_summary(fun.data = function(x) { return(c(y=max(to.plot2[variable=="expr",value]), label=length(x)))}, geom = "text",size=3, data=to.plot2[variable=="expr"]) +

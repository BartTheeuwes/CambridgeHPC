...
tf_evidence.dt <- fread("/Users/argelagr/data/gastrulation_multiome_10x/test/results/rna_atac/rna_vs_chromvar_chip/pseudobulk/per_celltype/neural_crest_tf_evidence.txt", sep=",") %>%
  .[,c("gene","celltype","species")] %>% setnames("species","evidence")
# tf_evidence_colours <- 
...
#######################################################
## Polar plot per cell type, coloured by TF evidence ##
#######################################################
...
for (i in celltypes.to.plot) {
  ...
  p <- ggplot(to.plot, aes(x=gene, y=tf_marker_score, group=1)) +
    geom_point(aes(size=dot_size, fill=evidence), stat = 'identity', shape=21, color="black", alpha=0.9) +
    ggrepel::geom_text_repel(data=to.plot.text, aes(label=gene), size=3, max.overlaps=Inf) +
  ...
  outfile <- file.path(io$outdir,sprintf("%s_rna_vs_chromvar_chip_%s_polar_plots_tf_evidence.pdf",i,opts$motif_annotation))

...
# I/O
io$outdir <- paste0(io$basedir,"/results/rna_atac/rna_vs_chromvar/trajectories/endoderm_trajectory")
...
io$pseudotime <- "/Users/ricard/data/gastrulation_multiome_10x/results/rna/trajectories/endoderm_trajectory/endoderm_trajectory.txt.gz"
trajectory.dt <- fread(io$pseudotime)

# Load highly variable along the NMP trajectory
io$hvgs <- "/Users/ricard/data/gastrulation_multiome_10x/results/rna/trajectories/endoderm_trajectory/hvgs.rds"
...
fwrite(chromvar_rna_dt, sprintf("%s/chromvar_rna_endoderm.txt.gz",io$outdir), quote=F, sep="\t", na="NA")
...
for (i in genes.to.plot) {
  ...
  pdf(sprintf("%s/individual_genes/%s_rna_chromvar_vs_pseudotime_endoderm.pdf",io$outdir,i), width=8, height=6)
  print(p)
  dev.off()
}
...
fwrite(cor.dt, sprintf("%s/correlation_results_endoderm.txt.gz",io$outdir), quote=F, sep="\t", na="NA")

...
p$add_argument('--test',    action="store_true",             help='Test mode? subset data')
args <- p$parse_args(commandArgs(TRUE))

## START TEST
# args <- list()
# args$sce <- file.path(io$basedir,"results_new/rna/trajectories/blood/blood_SingleCellExperiment.rds")
# args$metadata <- file.path(io$basedir,"results_new/rna/trajectories/blood/blood_sample_metadata.txt.gz")
# args$chromvar <- file.path(io$basedir,"results_new/atac/archR/chromvar_chip/chromVAR_chip_deviations_CISBP_archr.rds")
# args$trajectory <- file.path(io$basedir,"results_new/rna/trajectories/blood/blood_trajectory.txt.gz")
# args$trajectory_name <- "blood"
# args$denoise <- TRUE
# args$knn <- 25
# args$pca_rna <- file.path(io$basedir,"results_new/rna/dimensionality_reduction/sce/pca_features2500_pcs50.txt.gz")
# args$pca_atac <- file.path(io$basedir,"results_new/atac/archR/dimensionality_reduction/lsi_PeakMatrix_nfeatures25000_ndims50.txt.gz")
# args$outdir <- file.path(io$basedir,"results_new/rna_atac/rna_vs_chromvar_chip/trajectories/test")
# args$test <- TRUE
## END TEST
...
# Define cell types
opts$celltypes <- opts$celltypes[opts$celltypes%in%unique(sample_metadata$celltype)]

###################
## Load chromVAR ##
###################

chromvar.mtx <- readRDS(args$chromvar)[,sample_metadata$cell] %>% assay(.,"z")
...
to.plot <- chromvar_rna_dt %>%
  merge(sample_metadata[,c("cell","celltype.mapped_mnn")], by="cell") %>%

...
here::i_am("rna_atac/virtual_chipseq_library/create_virtual_chipseq_library.R")
...
args$tf2peak_cor <- file.path(io$basedir,sprintf("results_new/rna_atac/rna_vs_acc/pseudobulk/TFexpr_vs_peakAcc/%s_cor_TFexpr_vs_peakAcc.rds",args$motif_annotation)) 
...
args$outdir <- file.path(io$basedir,sprintf("results_new/rna_atac/virtual_chipseq/%s",args$motif_annotation))
args$threads <- 1
args$test <- FALSE
## END TEST ##
...
# Sanity checks
# stopifnot(rownames(atac_pseudobulk_PeakMatrix.se)%in%peak_metadata.dt$idx)
...
stopifnot(rownames(tf2peak_cor.se)%in%rownames(atac_pseudobulk_PeakMatrix.se))
...
genes <- intersect(colnames(tf2peak_cor.se),motif2gene.dt$gene)
...
tf2peak_cor.se <- tf2peak_cor.se[,genes]
...
atac_pseudobulk_PeakMatrix.mtx <- assay(atac_pseudobulk_PeakMatrix.se[peaks,]) %>% round(3)

# unique(motifmatcher.mtx[,motif2gene.dt[gene=="GATA1",motif]])
# unique(motifmatcher.mtx[,motif2gene.dt[gene=="TAL1",motif]])
...
virtual_chip.dt <- opts$TFs %>% map(function(i) {
  ...
  if (length(peaks)>=args$min_number_peaks) {
    ...
    max_accessibility_score <- apply(atac_pseudobulk_PeakMatrix.mtx[peaks,],1,max) %>% round(2)
...
assay(motifmatcher_chip.se) <- virtual_chip.mtx

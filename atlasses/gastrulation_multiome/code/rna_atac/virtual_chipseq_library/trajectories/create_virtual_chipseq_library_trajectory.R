...
args$tf2peak_cor <- file.path(io$basedir,sprintf("results_new/rna_atac/rna_vs_acc/trajectories/%s/TFexpr_vs_peakAcc_%s_%s.rds",args$trajectory_name,args$motif_annotation,args$trajectory_name))
# args$atac_peak_stats <- file.path(io$basedir,"results_new/atac/archR/feature_stats/PeakMatrix_stats.txt.gz")
args$atac_peak_matrix_pseudobulk <- file.path(io$basedir,"results_new/atac/archR/pseudobulk/celltype.mapped_mnn/pseudobulk_PeakMatrix_summarized_experiment.rds")
...
# Load peak stats
# atac_peak_stats.dt <- fread(args$atac_peak_stats)
# atac_peak_stats.mtx <- atac_peak_stats.dt[,c("feature","mean_pseudobulk")] %>% matrix.please

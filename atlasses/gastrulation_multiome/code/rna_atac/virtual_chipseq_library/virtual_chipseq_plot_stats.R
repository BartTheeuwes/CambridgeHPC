...
# I/O
io$virtual_chip.dir <- file.path(io$basedir,sprintf("results_new/rna_atac/virtual_chipseq/%s",opts$motif_annotation))
...
io$outdir <- file.path(io$basedir,sprintf("results_new/rna_atac/virtual_chipseq/%s/stats",opts$motif_annotation)); dir.create(io$outdir, showWarnings = F)
...
stopifnot(opts$TFs%in%colnames(virtual_chip.mtx))

virtual_chip_filt.mtx <- virtual_chip.mtx[,opts$TFs]
...
# sum(virtual_chip_filt.mtx[,"SOX10"]>=0.15)
...
to.plot <- seq.ranges %>% map(function(j) {
  data.table(
    tf = opts$TFs,
    min_score = j,
    N_negative = colSums(virtual_chip_filt.mtx<=(-j)),
    N_positive = colSums(virtual_chip_filt.mtx>=j)
  )

...
io$outdir <- file.path(io$basedir,"results_new/rna_atac/virtual_chipseq/CISBP/stats")
...
to.plot.compare_models <- foo %>%
  ...
  # Model 2: DNA + ATAC
  .[,score_dna_atac:=minmax.normalisation(max_accessibility_score * motif_score), by="chip_anno"] %>% 
  # Model 3: DNA + ATAC + RNA
  .[,score_rna_atac:=minmax.normalisation(score_rna_atac), by="chip_anno"] %>%
  # Discretise values
  ...
  .[!is.na(predicted_value) & N>=5]


p <- ggline(to.plot.compare_models, x="predicted_value", y="log_value", color="model", plot_type="b") +
  facet_wrap(~chip_anno, scales="free_y") +
  scale_color_brewer(palette="Dark2", labels = c("DNA", "DNA + ATAC", "DNA + ATAC + RNA")) +
...
# pdf(file.path(io$outdir,"insilico_vs_chipseq_lineplot.pdf"), width=8, height=4)
print(p)
# dev.off()
...
pdf(file.path(io$outdir,"insilico_vs_chipseq_lineplot_negative_values.pdf"), width=6.5, height=5)
print(p)
dev.off()

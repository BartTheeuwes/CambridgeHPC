...
io$virtual_chip_dir.prefix <- "/Users/argelagr/data/gastrulation_multiome_10x/results_new/rna_atac/virtual_chipseq/CISBP"
io$chip_peaks.files <- c(
  # "CDX2" = file.path(io$chip_dir.prefix, "Cdx2_NMP_invitro/macs2/cdx2_chip_peaks.narrowPeak.gz"),
  "TAL1" = file.path(io$chip_dir.prefix, "TAL1_blood_progenitors/fastq/HP_Tal1_macs2_broad/HP_Tal1_peaks.broadPeak"),
  # "GATA1" = file.path(io$chip_dir.prefix, "GATA1_blood_progenitors/macs2/Gata1_chip_peaks.narrowPeak.gz"),
  # "FOXA2" = file.path(io$chip_dir.prefix, "FOXA2_ESC_endoderm_differentiation/macs2/foxa2_chip_peaks.narrowPeak.gz"),
  # "GATA4" = file.path(io$chip_dir.prefix, "GATA4_ESC_mesoderm_differentiation/macs2/GATA4_peaks.narrowPeak.gz"),
...
chip_insilico.dt <- opts$TFs %>% map(function(i) {
  fread(sprintf("%s/%s.bed.gz",io$virtual_chip_dir.prefix,i)) %>%
    setnames(c("chr","start","end","score")) %>%
    .[,id:=sprintf("%s:%s-%s",chr,start,end)] %>%
    .[score>=0] %>%
    .[,tf:=i] %>%
...
ggplot(roc.dt, aes_string(x="fpr", y="tpr", color="tf")) +
  geom_line(size=1) +

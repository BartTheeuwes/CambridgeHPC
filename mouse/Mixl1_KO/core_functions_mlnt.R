...
load_data = function(normalise = TRUE, remove_doublets = FALSE, remove_stripped = FALSE, load_corrected = FALSE){
  ...
  #counts = readMM("/rds/project/bg200/rds-bg200-hphi-gottgens/users/bt392/mouse/Mixl1_KO/mlnt/2_calledCells/raw_counts.mtx")
  counts = readRDS("/rds/project/bg200/rds-bg200-hphi-gottgens/users/bt392/mouse/Mixl1_KO/mlnt/3_QC/raw_counts.rds")
  genes = read.table("/rds/project/bg200/rds-bg200-hphi-gottgens/users/bt392/mouse/Mixl1_KO/mlnt/2_calledCells/genes.tsv", stringsAsFactors = F)
  meta = read.table("/rds/project/bg200/rds-bg200-hphi-gottgens/users/bt392/mouse/Mixl1_KO/mlnt/3_QC/meta.tab", header = TRUE, sep = "\t", stringsAsFactors = FALSE, comment.char = "$")
  ...
  if(normalise){
    sfs = read.table("/rds/project/bg200/rds-bg200-hphi-gottgens/users/bt392/mouse/Mixl1_KO/mlnt/4_norm/sizefactors.tab", stringsAsFactors = F)[,1]
    sizeFactors(sce) = sfs
  ...
  if(load_corrected){
    corrected = readRDS("/rds/project/bg200/rds-bg200-hphi-gottgens/users/mlnt2/PhD_MT06/corrected_pcas.rds")
...
getHVGs <- function(sce, min.mean = 1e-3, sparse_matrix = FALSE){
 if(sparse_matrix == FALSE){
   ...
   trend  <- scran::fitTrendVar(sce, loess.args = list(span = 0.05))
 }
